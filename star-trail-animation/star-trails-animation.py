...
def make_end_frame(config):
    ...
    for i in range(nsample):
        ...
        if np.all(diffs > 0) or np.all(diffs < 0):
            ax.plot(galon, galat, c='w', lw=trailwidth, alpha=config['magscaling_trails'][i],
...
def parse_command_line_arguments():
    ...
    parser.add_argument('--exposure', type=float, nargs='?', default=4,
    ...
    parser.add_argument('--nstars_max', type=int, nargs='?', default=2000,
                        help="""Max number of stars to plot (default 2000)""")
    parser.add_argument('--max_alpha', type=float, nargs='?', default=0.4,
    ...
    parser.add_argument('--rngseed', type=int, nargs='?', default=53949896,
